...
#define WARP_DIM 32
#define NUM_TILES(N_KEYS) (N_KEYS-1)/TILE_DIM+1
...
inline void init(integer **keys, uint16_t **notCoprime, integer **d_keys, uint16_t **d_notCoprime, const char *filename,
                 const int numKeys);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < num_tiles; ++i) {
        for (int j = i; j < num_tiles; ++j) {
            ...
            cudaSafe(cudaMemcpy(notCoprime, d_notCoprime, BLKS_PER_TILE * BLKS_PER_TILE * sizeof(uint16_t),
